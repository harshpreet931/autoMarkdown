...
export class TokenCounter {
  ...
  private static generateRecommendations(
    tokens: number,
    compatible: LLMInfo[],
    incompatible: LLMInfo[]
  ): string[] {
    const recommendations: string[] = [];

    if (compatible.length === 0) {
      recommendations.push(
        '⚠️  Output too large for all popular LLMs! Consider using filters to reduce size.'
      );
      ...
      recommendations.push(
        '⚠️  Limited LLM compatibility. Consider reducing output size for broader support.'
      );
